...
if(part){parts.push(part)}return parts}const CSS_BUILD_ATTR="css-docs";/**
                                     * Return the polymer-css-docs "docs type" applied to this element
                                     *
...
const attrValue=element.getAttribute(CSS_BUILD_ATTR);if(attrValue){element.__cssBuild=attrValue}else{const buildComment=getBuildComment(element);if(""!==buildComment){// remove docs comment so it is not needlessly copied into every element instance
removeBuildComment(element)}element.__cssBuild=buildComment}}return element.__cssBuild||""}/**
   * Check if the given element, either a <template> or <style>, has been processed
   * by polymer-css-docs.
   *
   * If so, then we can make a number of optimizations:
   * - polymer-css-docs will decompose mixins into individual CSS Custom Properties,
   * so the ApplyShim can be skipped entirely.
   * - Under native ShadowDOM, the style text can just be copied into each instance
   * without modification
   * - If the docs is "shady" and ShadyDOM is in use, the styling does not need
   * scoping beyond the shimming of CSS Custom Properties
   *
   * @param {!HTMLElement} element
   * @return {boolean}
   */function elementHasBuiltCss(element){return""!==getCssBuild(element)}/**
   * For templates made with tagged template literals, polymer-css-docs will
   * insert a comment of the form `<!--css-docs:shadow-->`
   *
   * @param {!HTMLElement} element
   * @return {string}
   ...
   * Check if the css docs status is optimal, and do no unneeded work.
   *
   * @param {string=} cssBuild CSS docs status
   * @return {boolean} css docs is optimal or not
...
`,is:"paper-card",properties:{/**
   ...
   */_isHidden:function(image){return image?"false":"true"},_headingChanged:function(heading){var currentHeading=this.getAttribute("heading"),currentLabel=this.getAttribute("aria-label");if("string"!==typeof currentLabel||currentLabel===currentHeading){this.setAttribute("aria-label",heading)}},_computeHeadingClass:function(image){return image?" over-image":""},_computeAnimated:function(animatedShadow){return animatedShadow}});Polymer({is:"paper-icon-button",_template:html`
    <style>
      :host {
        display: inline-block;
        position: relative;
...
`,is:"paper-item",behaviors:[PaperItemBehavior]});Polymer({_template:html`
    ...
    <slot></slot>
`,is:"paper-listbox",behaviors:[IronMenuBehavior],/** @private */hostAttributes:{role:"listbox"}});const PaperSpinnerBehavior={properties:{/**
...
</dom-module>`;document.head.appendChild($_documentContainer$2.content);const template$a=html`
...
`;template$a.setAttribute("strip-whitespace","");/**
                                                 ...
                                                 @hero hero.svg
                                                 @demo demo/index.html
                                                 */Polymer({_template:template$a,is:"paper-spinner",behaviors:[PaperSpinnerBehavior]});class LoginPage extends PolymerElement{static get template(){return html`
      <style>
        :host {
          display: block;
        }
...
`;console.log("camsads");baseStyle.register("base-style");class LoadingComponent extends PolymerElement{static get template(){return html`
      <style>
        :host {
          display: block;
        }
      ...
      <paper-spinner active></paper-spinner>
    `}static get properties(){return{isStillLoading:{type:Boolean,notify:!0}}}constructor(){super();afterNextRender(this,()=>{setTimeout(()=>{this.set("isStillLoading",!1)},3e3)})}}window.customElements.define("loading-component",LoadingComponent);class HiringApp extends PolymerElement{static get template(){return html`
    ...
    `}static get properties(){return{login:{type:Boolean,value:!0},loading:{type:Boolean,value:!0,observer:"_loadingChange"}}}_loadingChange(newlValue,oldValue){if(!newlValue){setTimeout(()=>{const animation=this.shadowRoot.querySelector("#animate-container").animate([{transform:"translateY(100%)",opacity:1,easing:"ease-out"},{transform:"translateY(0)",opacity:1,easing:"ease-in"},{transform:"translateY(0)",opacity:1}],{duration:200});this.shadowRoot.querySelector("#animate-container").style.opacity="1"},0)}}}window.customElements.define("hiring-app",HiringApp)});